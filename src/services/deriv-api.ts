...
export interface TickData {
  tick: {
    ...
    epoch: number;
    quote: number;
    quote_raw?: string; // Added to preserve raw string with trailing zeros
    ask: number;
    bid: number;
  };
}
...
export interface TickHistoryResponse {
  history: {
    prices: number[];
    prices_raw?: string[]; // Added to preserve raw prices from history
    times: number[];
  };
}
...
// FIXED: Enhanced last digit extraction that properly handles trailing zeros
export function getLastDigit(price: number | string, rawPrice?: string): number {
  // Priority 1: Use raw price string if provided (most accurate)
  if (rawPrice !== undefined && rawPrice !== null) {
    ...
    if (priceStr.includes('e') || priceStr.includes('E')) {
      ...
      const decimalMatch = fixedStr.match(/\.(\d+)$/);
      if (decimalMatch) {
    ...
    // Check if it's a whole number (ends with .0)
    if (priceStr.includes('.') && decimalPart.length === 0) {
      return 0;
    }
  ...
  // Priority 2: Convert number to string with preserved decimal places
  ...
  // Convert to string with up to 10 decimal places to capture trailing zeros
  const priceStr = numericPrice.toFixed(10);
  const decimalMatch = priceStr.match(/\.(\d+)$/);
  
  if (decimalMatch && decimalMatch[1].length > 0) {
    const decimalPart = decimalMatch[1];
    // Remove trailing zeros for detection? No - we want the actual last digit
    // But if all are zeros, the last digit is 0
    const lastChar = decimalPart.charAt(decimalPart.length - 1);
    const digit = parseInt(lastChar, 10);
    return isNaN(digit) ? 0 : digit;
  }
...
class DerivAPI {
  ...
  connect(): Promise<void> {
    ...
    this.connectPromise = new Promise((resolve, reject) => {
      ...
      this.ws.onmessage = (event) => {
        // Capture raw message to preserve trailing zeros
        ...
        // Preserve raw quote strings for tick data
        if (data.tick && data.tick.quote !== undefined) {
          data.tick.quote_raw = data.tick.quote.toString();
        }
        
        // Also preserve raw prices in tick history responses
        if (data.history && data.history.prices && Array.isArray(data.history.prices)) {
          data.history.prices_raw = data.history.prices.map((p: any) => p.toString());
        }
  ...
  async getTickHistory(symbol: string, count: number = 100): Promise<TickHistoryResponse> {
    ...
    // Preserve raw price strings for accurate digit extraction
    if (response.history && response.history.prices) {
      response.history.prices_raw = response.history.prices.map((p: any) => p.toString());
    }
...
// FIXED: Scanner class that properly handles trailing zeros
export class LastDigitScanner {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (history.history && history.history.prices) {
        // Use raw prices if available for accurate digit extraction
        const rawPrices = history.history.prices_raw || history.history.prices.map(p => p.toString());
        for (let i = 0; i < history.history.prices.length; i++) {
          const price = history.history.prices[i];
          const rawPrice = rawPrices[i];
          this.processTick(price, rawPrice);
        }
      }
    } catch (err) {
    ...
    this.tickHandler = (data: any) => {
      if (data.tick && data.tick.symbol === this.symbol) {
        // Use the raw quote string if available
        const rawQuote = data.tick.quote_raw || data.tick.quote.toString();
        this.processTick(data.tick.quote, rawQuote);
      }
    };
  ...
  // FIXED: Process tick with raw price to preserve trailing zeros
  private processTick(price: number, rawPrice?: string): void {
    if (!this.isActive) return;
    
    this.lastPrice = price;
    // Use the raw price string for accurate last digit extraction
    const lastDigit = safeLastDigit(price, rawPrice);
...
export function DigitDistributionDisplay({ state }: { state: ScannerState }) {
  // This is a React component - make sure React is imported in your app
